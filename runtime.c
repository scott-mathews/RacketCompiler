 ...
 * Pointer 2 -> Vector 2
 * ...
 * Pointer n ** rootstack_end
 * */

void cheney(int64_t** rootstack_ptr)
{

	// Initialize free pointer to the start of the tospace
  int64_t* scan_ptr = tospace_begin;
  free_ptr = tospace_begin;

	// for each rootstack pointer, copy the vector it contains to the tospace
	// (cheney pt. 1)
  for (int64_t** scanner = rootstack_begin; scanner != rootstack_ptr; ++scanner)
	{
    copy_vector(scanner);
  }

	// for each vector now in tospace, copy the vectors it points to into
	// the tospace.
	// (cheney pt. 2)
  while (scan_ptr != free_ptr) 
	{
		// get tag of current tospace vector
    int64_t tag = *scan_ptr;

		// get location of next tospace vector
    int len = get_length(tag);
    int64_t* next = scan_ptr + len + 1;

		// get pointer mask from the tag of current vector
    int64_t ptr_mask = get_ptr_bitfield(tag);

		// Scan thru vector and pointer mask simultaneously
    scan_ptr++;
    while(scan_ptr != next)
		{
			// if the pointer mask indicates a vector at current location
			// copy that vector
      if ((ptr_mask & 1) == 1)
			{
        copy_vector((int64_t**) scan_ptr);
			...
			// increment pointer mask and vector location
      ptr_mask = ptr_mask >> 1;
      scan_ptr += 1;
    }
  }
	...
	// Swap the begin delimeters for tospace and fromspace
	int64_t* swapper = tospace_begin;
	tospace_begin = fromspace_begin;
	fromspace_begin = swapper;

	// Swap the end delimeters for tospace and fromspace
	swapper = tospace_end;
	tospace_end = fromspace_end;
	fromspace_end = swapper;
}
...
{
	// Pull vector pointer from the rootstack	
  int64_t* old_vector_ptr = vector_ptr_loc[0];
  int old_tag = any_tag((int64_t)old_vector_ptr);
	
	// If we find non-pointer junk in the place of the old pointer
	// move on
  ...
  int64_t tag = *old_vector_ptr;

	// If the tag is forwarding, then update the current tag
  if (is_forwarding(tag)) 
	{
    *vector_ptr_loc = (int64_t*) (tag | old_tag);
  } 

	// otherwise, we need to copy the vector
	else 
	{
		// we will place the new vector at the first free location
    int64_t* new_vector_ptr = free_ptr;

		// copy over old vector contents
    for (int i=0; i!=get_length(tag)+1; i++)
		{
      new_vector_ptr[i] = old_vector_ptr[i];
    }
		
		// move free_ptr to next free location
    free_ptr = free_ptr + get_length(tag) + 1;

		// make the old vector tag a pointer to the new vector
    old_vector_ptr[0] = (int64_t) new_vector_ptr;

		// update the tag of the new pointer
